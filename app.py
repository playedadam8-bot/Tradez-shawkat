...
if st.button("🚀 Generate Live Signal"):
    with st.spinner(f"Fetching real Yahoo Finance live market data & running GPT-4o analysis for {selected_pair}..."):
        try:
            ...
            # FIXED ENTRY TIME: Local time (UTC+5) plus 5 seconds buffer into the future
            ...
            if response.status_code == 200:
                ...
                if "choices" in result and len(result["choices"]) > 0:
                    ...
                    # 1. DISPLAY UP/DOWN IMAGE BANNER FIRST ON TOP (Dynamic Green/Red)
                    if signal == "CALL":
                        st.markdown(
                            """
                    ...
                    # 2. DISPLAY ENTRY TIME RIGHT AFTER BANNER (Accurate Future Time UTC+5)
                    st.markdown(f"### ⚡ Target Entry Time: `{formatted_time}` (UTC+5)")
