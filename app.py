...
TIMEFRAMES = ["5s", "10s", "15s", "20s", "30s", "45s", "1min", "2min", "3min", "5min", "10min", "15min"]

# 4. Sidebar Controls
st.sidebar.header("⚙️ Live Market Control Panel")
selected_pair = st.sidebar.selectbox("Select Live Currency Pair", LIVE_PAIRS)
selected_timeframe = st.sidebar.selectbox("Select Expiry Timeframe", TIMEFRAMES, index=6)

# 5. Main UI
st.subheader(f"Selected Asset: **{selected_pair}** | Timeframe: **{selected_timeframe}**")
st.info("Click the button below to fetch current technical indicators and generate a real-time signal via OpenRouter AI without needing screenshots.")
...
if st.button("🚀 Generate Live Signal"):
    with st.spinner(f"Analyzing live market data for {selected_pair} ({selected_timeframe})..."):
        try:
            ...
            Tasks:
            1. Analyze the expected short-term market momentum, trend behavior, and typical technical conditions for {selected_pair} at this timeframe.
