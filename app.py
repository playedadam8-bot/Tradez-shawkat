...
# 2. All API Keys Pool (Will cycle through them automatically)
API_KEYS = [
    ...
    with st.spinner(f"AI Analyzing {selected_pair_key} Cycles..."):
        try:
            ...
            if success and response.status_code == 200:
                ...
                st.error(f"API Error {response.status_code if response else 'Unknown'}: {response.text if response else 'No response'}")
