...
# Flags Mapping with proper spacing to prevent icon distortion
FLAG_MAP = {
...
if st.button("🚀 GENERATE TRADING SIGNAL"):
    with st.spinner(f"AI Analyzing {selected_pair_key} Cycles..."):
        try:
            ...
            # Use index [-2] (the last fully closed candle) to avoid flat live Doji ticks
            ...
            # Latest tick for execution price
            live_price = float(todays_data['Close'].iloc[-1])

            # Clear Bullish / Bearish evaluation
            ...
            # Accurate Local Time Handling (PKT / Asia/Karachi or system local fallback)
            try:
            ...
            if response.status_code == 200:
                ...
                if "choices" in res_json and len(res_json["choices"]) > 0:
                    ...
                    if json_match:
                        ...
                        clean_pair_code = selected_pair_key.replace("/", "")
                        st.markdown(f"""
