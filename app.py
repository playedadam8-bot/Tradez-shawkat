...
# Helper function to generate a visual candle image
def generate_candle_image(o, c, h, l):
    ...
    # Wick (High to Low)
    ax.plot([1, 1], [l, h], color=color, linewidth=2)
    
    # Body (Open to Close)
    body_bottom = min(o, c)
    body_height = abs(c - o)
    if body_height == 0:
        body_height = 0.00005 # Minimal thickness for Doji
        
    rect = patches.Rectangle((0.7, body_bottom), 0.6, body_height, facecolor=color, edgecolor=color)
    ax.add_patch(rect)
    
    ax.set_xlim(0, 2)
    ax.set_ylim(l - (h-l)*0.1 if h!=l else l-0.001, h + (h-l)*0.1 if h!=l else l+0.001)
    ax.axis('off')
...
if st.button("🚀 GENERATE TRADING SIGNAL"):
    with st.spinner(f"AI Analyzing {selected_pair} Cycles..."):
        try:
            ...
            # Determine last candle direction & color
            ...
            # Synced Real-Time generation (Using strict system matching offset)
            ...
            if response.status_code == 200:
                ...
                if "choices" in res_json and len(res_json["choices"]) > 0:
                    ...
                    if json_match:
                        ...
                        # Metrics Display & Candle Visualizer Side-by-Side
                        ...
                        with col_m2:
                            st.markdown("<p style='text-align:center; font-family:Rajdhani; color:#4facfe; margin-bottom:0;'>LAST CANDLE VISUAL</p>", unsafe_allow_html=True)
                            candle_img = generate_candle_image(current_open, current_close, current_high, current_low)
