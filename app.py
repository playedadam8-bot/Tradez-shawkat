...
if st.button("🚀 GENERATE TRADING SIGNAL"):
    with st.spinner(f"AI Analyzing {selected_pair} Cycles..."):
        try:
            ...
            # Detailed prompt to guarantee proper JSON response
            prompt = f"""
            ...
            Provide a directional signal ("CALL" or "PUT") and an estimated close price.
            Output ONLY raw JSON format matching this exact structure, with no markdown tags:
            {{"signal": "CALL", "signal_close_price": "{current_close:.5f}"}}
            """
            ...
            if response.status_code == 200:
                ...
                if "choices" in res_json and len(res_json["choices"]) > 0:
                    ...
                    # Clean potential markdown block wrappers safely
